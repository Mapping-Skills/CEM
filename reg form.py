...
def insert():
    try:
        ...
        if (user_id.get() == "" or password.get() == "" or first_name.get() == "" or last_name.get() == "" or email.get() == "" or var.get() == "" or reg_type.get() == "" or con_num.get()==''):
            print('empty input')
            ourMessage = 'Fill all the Details'
            ...
            cur.execute('''INSERT INTO stu_reg VALUES(?,?,?,?,?,?,?,?)''', (first_name.get(),last_name.get(),user_id.get(),password.get(),gender,con_num.get(),reg_type.get(),email.get()))
            connec.commit()
            connec.close()
            ourMessage='Data Addded'
        ...
        print("Error :" , e)
        messageVar = Message(main, text= e)
        messageVar.config(bg='lightgreen')
        messageVar.pack()
        main.mainloop()
